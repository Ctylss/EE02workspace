                ...
                <div class="form-group">
                    ...
                    <select id="supplier" name="supplierId" required>
                        ...
                        <% for (Supplier s : supplierList) {
                            String selected = (order.getSupplier() != null && s.getSupplierId() == order.getSupplier().getSupplierId())
                                              ? "selected" : "";
                        %>
                            <option value="<%= s.getSupplierId() %>" <%= selected %>>
                                <%= s.getSupplierName() %>
                            </option>
                        <% } %>
                    </select>
                    ...
                    <tbody>
                        <% for (OrderItem item : items) { %>
                            <tr>
                                ...
                                <td>
                                    <input type="number" name="unitPrice[]" step="0.01" value="<%= item.getUnitPrice() %>" required>
                                </td>
                            </tr>
                        <% } %>
                    </tbody>
