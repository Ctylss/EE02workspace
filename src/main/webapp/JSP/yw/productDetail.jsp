...
<%-- 將 pageTitle 直接設定到 request 作用域，這樣在 <title> 標籤中可以直接使用 ${pageTitle} --%>
<c:set var="isNew" value="${empty requestScope.product}"/>
<c:set var="formAction" value="${isNew ? 'add' : 'update'}"/>
<c:set var="pageTitle" value="${isNew ? '新增產品' : '編輯產品'}" scope="request"/> <%-- 修正：直接設定作用域為 request --%>
...
  <title><c:out value="${pageTitle}"/></title> <%-- 直接使用 EL 從 request 作用域取得 pageTitle --%>
...
      <div class="form-text text-muted">如果無類別，可以留空。</div>
    </div>
